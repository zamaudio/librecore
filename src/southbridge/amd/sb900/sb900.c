...
static void sb900_disable_usb(u8 disable)
{
	u8 reg8;
...
void sb900_enable(device_t dev)
{
	...
	case PCI_DEVFN(0x12, 0):
		if (dev->enabled == 0)
			sb900_disable_usb(USB_EN_DEVFN_12_0);
	case PCI_DEVFN(0x12, 2):	/* Fall through */
		if (dev->enabled == 0)
			sb900_disable_usb(USB_EN_DEVFN_12_2);
		break;
	case PCI_DEVFN(0x13, 0):
		if (dev->enabled == 0)
			sb900_disable_usb(USB_EN_DEVFN_13_0);
	case PCI_DEVFN(0x13, 2):	/* Fall through */
		if (dev->enabled == 0)
			sb900_disable_usb(USB_EN_DEVFN_13_2);
		break;
	case PCI_DEVFN(0x16, 0):
		if (dev->enabled == 0)
			sb900_disable_usb(USB_EN_DEVFN_16_0);
	case PCI_DEVFN(0x16, 2):	/* Fall through */
		if (dev->enabled == 0)
			sb900_disable_usb(USB_EN_DEVFN_16_2);
		break;
	default:
		break;
	}
...
static void sb900_init_acpi_ports(void)
{
...
static void sb900_init(void *chip_info)
{
	sb900_init_acpi_ports();
}

struct chip_operations southbridge_amd_sb900_ops = {
	CHIP_NAME("AMD SB900")
